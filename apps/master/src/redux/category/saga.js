...
import service from '@shared/services/common/categoryService';
import modelHelper from '@shared/lib/helpers/modelHelper';

//#region Init
export function* initGet() {
  try {
    let [resTypes] = yield call(initGetDataAsync);
    yield put(actions.initSet({ types: resTypes.data }));
...
async function initGetDataAsync() {
  return await Promise.all([service.getTypesPromise()]);
}
//#endregion
