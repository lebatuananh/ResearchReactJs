import { all, takeEvery, takeLatest, put, call, fork, delay } from 'redux-saga/effects';
import { intl } from '@shared/lib/core/Localization/IntlGlobalProvider';
import actions from './actions';
import service from '@shared/services/master/list/wardService';
...
//import zoneService from '@shared/services/master/list/zoneService';

//#region Init
export function* initGet() {
  try {
    let [resCountries] = yield call(initGetDataAsync);
    yield put(actions.initSet({ countries: resCountries.data }));
...
async function initGetDataAsync() {
  return await Promise.all([stateProvinceService.getCountriesPromise(false)]);
}
//#endregion
...
export function* createCountryChange(action) {
  try {
    if (action.payload) {
      ...
      yield put(actions.createDistrictSet([]));
      yield put(actions.createStateProvinceSetSuccess());
    }
  } catch (error) {
...
export function* createStateProvinceChange(action) {
  try {
    if (action.payload) {
      let result = yield call(districtService.getByStateProvinceIdAsync, action.payload);
      if (!result) {
        yield put(actions.createDistrictSetError(intl.formatMessage({ id: 'common.notify.error.noResults' })));
        yield put(actions.createDistrictResetError());
      } else if (result.success) {
        const { data } = result;
        yield put(actions.createDistrictSet(data));
        yield put(actions.createDistrictSetSuccess());
      } else {
        yield put(actions.createDistrictSetError(result.message));
        yield put(actions.createDistrictResetError());
      }
    } else {
      yield put(actions.createDistrictSet([]));
    ...
    yield put(actions.createDistrictSetError(error.message));
    yield put(actions.createDistrictResetError());
  }
}
//#endregion
...
export function* editCountryChange(action) {
  try {
    if (action.payload) {
      ...
      yield put(actions.editDistrictSet([]));
      yield put(actions.editStateProvinceSetSuccess());
    }
  } catch (error) {
...
export function* editStateProvinceChange(action) {
  try {
    if (action.payload) {
      let result = yield call(districtService.getByStateProvinceIdAsync, action.payload);
      if (!result) {
        yield put(actions.editDistrictSetError(intl.formatMessage({ id: 'common.notify.error.noResults' })));
        yield put(actions.editDistrictResetError());
      } else if (result.success) {
        const { data } = result;
        yield put(actions.editDistrictSet(data));
        yield put(actions.editDistrictSetSuccess());
      } else {
        yield put(actions.editDistrictSetError(result.message));
        yield put(actions.editDistrictResetError());
      }
    } else {
      yield put(actions.editDistrictSet([]));
    ...
    yield put(actions.editDistrictSetError(error.message));
    yield put(actions.editDistrictResetError());
  }
}
//#endregion
...
//#region search
export function* searchCountryChange(action) {
  try {
    if (action.payload) {
      let result = yield call(stateProvinceService.getByCountryIdAsync, action.payload);
      if (!result) {
        yield put(actions.searchStateProvinceSetError(intl.formatMessage({ id: 'common.notify.error.noResults' })));
        yield put(actions.searchStateProvinceResetError());
      } else if (result.success) {
        const { data } = result;
        yield put(actions.searchStateProvinceSet(data));
        yield put(actions.searchStateProvinceSetSuccess());
      } else {
        yield put(actions.searchStateProvinceSetError(result.message));
        yield put(actions.searchStateProvinceResetError());
      }
    } else {
      yield put(actions.searchStateProvinceSet([]));
      yield put(actions.searchDistrictSet([]));
      yield put(actions.searchStateProvinceSetSuccess());
    }
  } catch (error) {
    yield put(actions.searchStateProvinceSetError(error.message));
    yield put(actions.searchStateProvinceResetError());
  }
}

export function* searchStateProvinceChange(action) {
  try {
    if (action.payload) {
      let result = yield call(districtService.getByStateProvinceIdAsync, action.payload);
      if (!result) {
        yield put(actions.searchDistrictSetError(intl.formatMessage({ id: 'common.notify.error.noResults' })));
        yield put(actions.searchDistrictResetError());
      } else if (result.success) {
        const { data } = result;
        yield put(actions.searchDistrictSet(data));
        yield put(actions.searchDistrictSetSuccess());
      } else {
        yield put(actions.searchDistrictSetError(result.message));
        yield put(actions.searchDistrictResetError());
      }
    } else {
      yield put(actions.searchDistrictSet([]));
    ...
    yield put(actions.searchDistrictSetError(error.message));
    yield put(actions.searchDistrictResetError());
  }
}
//#endregion
...
export default function* wardSaga() {
  yield all([
    ...
    yield takeLatest(actions.SEARCH_COUNTRY_CHANGE, searchCountryChange),
    ...
    yield takeLatest(actions.SEARCH_STATE_PROVINCE_CHANGE, searchStateProvinceChange),
    yield takeLatest(actions.CREATE_STATE_PROVINCE_CHANGE, createStateProvinceChange),
    yield takeLatest(actions.EDIT_STATE_PROVINCE_CHANGE, editStateProvinceChange),
  ]);
}
