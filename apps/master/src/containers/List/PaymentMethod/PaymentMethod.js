...
import actions from '../../../redux/list/paymentMethod/actions';
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${paymentMethodApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
          ...
          <Row style={rowStyle} gutter={gutter}>
            <Col span={9} style={colStyle}>
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <FormattedMessage id='common.paymentMethod' />
                </Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          ...
          >
            <Column dataField='Code' dataType='string' caption={intl.formatMessage({ id: 'admin.paymentMethods.fields.code' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.paymentMethods.fields.name' })} fixed={true} width={220} />
            <Column
              dataField='EnabledARPay'
              dataType='boolean'
              caption={intl.formatMessage({ id: 'admin.paymentMethods.fields.enabledARPay' })}
              width={220}
              cellRender={CellActive}
            />
            <Column
              dataField='EnabledAPPay'
              dataType='boolean'
              caption={intl.formatMessage({ id: 'admin.paymentMethods.fields.enabledAPPay' })}
              width={220}
              cellRender={CellActive}
            />
          ...
          <ContextMenu
            ...
            target='#grdPaymentMethod .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.paymentMethods.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.paymentMethods.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    loading: state.PaymentMethod.loading,
    create: {
      modelGet: state.PaymentMethod.create.modelGet,
      modelPost: state.PaymentMethod.create.modelPost,
      result: state.PaymentMethod.create.result,
      error: state.PaymentMethod.create.error,
      loading: state.PaymentMethod.create.loading,
      visible: state.PaymentMethod.create.visible,
    },
    edit: {
      modelGet: state.PaymentMethod.edit.modelGet,
      modelPost: state.PaymentMethod.edit.modelPost,
      result: state.PaymentMethod.edit.result,
      error: state.PaymentMethod.edit.error,
      loading: state.PaymentMethod.edit.loading,
      visible: state.PaymentMethod.edit.visible,
    },
    deletes: {
      modelPost: state.PaymentMethod.deletes.modelPost,
      result: state.PaymentMethod.deletes.result,
      error: state.PaymentMethod.deletes.error,
      loading: state.PaymentMethod.deletes.loading,
      visible: state.PaymentMethod.deletes.visible,
    },
    activates: {
      modelPost: state.PaymentMethod.activates.modelPost,
      result: state.PaymentMethod.activates.result,
      error: state.PaymentMethod.activates.error,
    },
    grd: {
      loading: state.PaymentMethod.grd.loading,
    },
  };
}
