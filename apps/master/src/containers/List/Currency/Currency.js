...
import actions from '../../../redux/list/currency/actions';
...
class Currency extends React.Component {
  constructor(props) {
    ...
    // custom menu context => remove active
    const contextMenuData = ctxHelper.def.dataSource.filter((p) => p.id !== 'activates' && p.id !== 'deactivates');
    this.ctxMenu = {
      ref: React.createRef(),
      dataSource: contextMenuData, // ctxHelper.def.dataSource,
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${currencyApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
  ...
  componentDidUpdate() {
    const { create, edit, deletes, grd } = this.props;
        ...
        <LayoutContentWrapper>
          ...
          >
            <Column
              ...
              dataType='string'
              caption={intl.formatMessage({ id: 'admin.currencies.fields.currencyCode' })}
              fixed={true}
              minWidth={150}
            />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.currencies.fields.name' })} fixed={true} minWidth={220} />
            <Column dataField='Rate' dataType='number' caption={intl.formatMessage({ id: 'admin.currencies.fields.rate' })} minWidth={220} />
            <Column
              dataField='Published'
              dataType='boolean'
              caption={intl.formatMessage({ id: 'admin.currencies.fields.published' })}
              width={100}
          ...
          <ContextMenu
            ...
            target='#grdCurrency .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.currencies.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.currencies.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    create: {
      modelGet: state.Currency.create.modelGet,
      modelPost: state.Currency.create.modelPost,
      result: state.Currency.create.result,
      error: state.Currency.create.error,
      loading: state.Currency.create.loading,
      visible: state.Currency.create.visible,
    },
    edit: {
      modelGet: state.Currency.edit.modelGet,
      modelPost: state.Currency.edit.modelPost,
      result: state.Currency.edit.result,
      error: state.Currency.edit.error,
      loading: state.Currency.edit.loading,
      visible: state.Currency.edit.visible,
    },
    deletes: {
      modelPost: state.Currency.deletes.modelPost,
      result: state.Currency.deletes.result,
      error: state.Currency.deletes.error,
      loading: state.Currency.deletes.loading,
      visible: state.Currency.deletes.visible,
    },
    // activates: {
    //   modelPost: state.Currency.activates.modelPost,
    //   result: state.Currency.activates.result,
    //   error: state.Currency.activates.error,
    // },
    grd: {
      loading: state.Currency.grd.loading,
    },
  };
}
