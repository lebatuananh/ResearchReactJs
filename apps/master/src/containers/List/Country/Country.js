...
import actions from '../../../redux/list/country/actions';
import LayoutContentWrapper from '@shared/components-admin/utility/layoutWrapper';
import { Row, Col, Form, Input, Button, Select, Spin, Modal, Space, Breadcrumb, Tooltip, Cascader, Dropdown, Menu } from 'antd';
...
class Country extends React.Component {
      ...
      load: (loadOptions) => {
        ...
        const objParams = {
          ...
          ...{
            ...
            GlobalZoneId: frmSearch.getFieldValue('GlobalZoneId'),
            LanguageId: workContext.languageId(),
          },
        };
        let params = qs.stringify(objParams);
        return fetch(`${countryApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
  ...
  componentDidUpdate() {
    const { init, create, edit, deletes, activates, grd } = this.props;
    if (init.error) {
  ...
  render() {
    const { height, loading, create, edit, deletes, search, createReset, editReset } = this.props;
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
            ...
            <Col span={24} style={colStyle}>
              <Form id='frmSearch' ref={this.frmSearch.ref}>
                <Row gutter={gutter}>
                  ...
                  <Col span={8}>
                    <Tooltip title={<FormattedMessage id='common.globalZone' />}>
                      <Form.Item name='GlobalZoneId'>
                        <Select
          ...
          >
            <Column dataField='Code' dataType='string' caption={intl.formatMessage({ id: 'admin.countries.fields.code' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.countries.fields.name' })} fixed={true} width={220} />
            <Column dataField='ShortName' dataType='string' caption={intl.formatMessage({ id: 'admin.countries.fields.shortName' })} width={150} />
            <Column dataField='LocalName' dataType='string' caption={intl.formatMessage({ id: 'common.fields.localName' })} width={220} />
            <Column dataField='GlobalZone.Name' dataType='string' caption={intl.formatMessage({ id: 'common.globalZone' })} width={220} />
            ...
            <Column
              ...
              dataType='number'
              caption={intl.formatMessage({ id: 'common.fields.displayOrder' })}
              width={110}
              alignment='right'
            />
          ...
          <ContextMenu
            ...
            target='#grdCountry .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.countries.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.countries.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    init: state.Country.init,
    search: {
      globalZones: state.Country.search.globalZones,
    },
    create: {
      modelGet: state.Country.create.modelGet,
      modelPost: state.Country.create.modelPost,
      result: state.Country.create.result,
      error: state.Country.create.error,
      loading: state.Country.create.loading,
      visible: state.Country.create.visible,
    },
    edit: {
      modelGet: state.Country.edit.modelGet,
      modelPost: state.Country.edit.modelPost,
      result: state.Country.edit.result,
      error: state.Country.edit.error,
      loading: state.Country.edit.loading,
      visible: state.Country.edit.visible,
    },
    deletes: {
      modelPost: state.Country.deletes.modelPost,
      result: state.Country.deletes.result,
      error: state.Country.deletes.error,
      loading: state.Country.deletes.loading,
      visible: state.Country.deletes.visible,
    },
    activates: {
      modelPost: state.Country.activates.modelPost,
      result: state.Country.activates.result,
      error: state.Country.activates.error,
    },
    grd: {
      loading: state.Country.grd.loading,
    },
  };
}
