...
import actions from '../../../redux/list/sPProductType/actions';
...
class SPProductType extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${sPProductTypeApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
        ...
        <LayoutContentWrapper>
          ...
          <Row style={rowStyle} gutter={gutter}>
            <Col span={9} style={colStyle}>
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <FormattedMessage id='common.sPProductType' />
                </Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          ...
          >
            <Column
              dataField='Code'
              dataType='string'
              caption={intl.formatMessage({ id: 'admin.sPProductTypes.fields.code' })}
              fixed={true}
              minWidth={150}
              width={220}
            />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.sPProductTypes.fields.name' })} fixed={true} minWidth={220} />
            <Column
              dataField='Active'
              dataType='bool'
          ...
          <ContextMenu
            ...
            target='#grdSPProductType .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.sPProductTypes.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.sPProductTypes.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    loading: state.SPProductType.loading,
    create: {
      modelGet: state.SPProductType.create.modelGet,
      modelPost: state.SPProductType.create.modelPost,
      result: state.SPProductType.create.result,
      error: state.SPProductType.create.error,
      loading: state.SPProductType.create.loading,
      visible: state.SPProductType.create.visible,
    },
    edit: {
      modelGet: state.SPProductType.edit.modelGet,
      modelPost: state.SPProductType.edit.modelPost,
      result: state.SPProductType.edit.result,
      error: state.SPProductType.edit.error,
      loading: state.SPProductType.edit.loading,
      visible: state.SPProductType.edit.visible,
    },
    deletes: {
      modelPost: state.SPProductType.deletes.modelPost,
      result: state.SPProductType.deletes.result,
      error: state.SPProductType.deletes.error,
      loading: state.SPProductType.deletes.loading,
      visible: state.SPProductType.deletes.visible,
    },
    activates: {
      modelPost: state.SPProductType.activates.modelPost,
      result: state.SPProductType.activates.result,
      error: state.SPProductType.activates.error,
    },
    grd: {
      loading: state.SPProductType.grd.loading,
    },
  };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(SPProductType);
