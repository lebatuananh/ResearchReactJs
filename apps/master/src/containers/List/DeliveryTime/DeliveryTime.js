...
import actions from '../../../redux/list/deliveryTime/actions';
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${deliveryTimeApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
          ...
          <Row style={rowStyle} gutter={gutter}>
            <Col span={9} style={colStyle}>
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <FormattedMessage id='common.deliveryTime' />
                </Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          ...
          >
            <Column dataField='Code' dataType='string' caption={intl.formatMessage({ id: 'admin.deliveryTimes.fields.code' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.deliveryTimes.fields.name' })} fixed={true} width={220} />
            <Column dataField='MinValue' dataType='number' caption={intl.formatMessage({ id: 'admin.deliveryTimes.fields.minValue' })} width={150} />
            <Column dataField='MaxValue' dataType='number' caption={intl.formatMessage({ id: 'admin.deliveryTimes.fields.maxValue' })} width={150} />
            <Column dataField='TimeUnitText' dataType='string' caption={intl.formatMessage({ id: 'common.fields.deliveryTimeUnit' })} width={220} />
          ...
          <ContextMenu
            ...
            target='#grdDeliveryTime .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.deliveryTimes.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.deliveryTimes.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    loading: state.DeliveryTime.loading,
    create: {
      modelGet: state.DeliveryTime.create.modelGet,
      modelPost: state.DeliveryTime.create.modelPost,
      result: state.DeliveryTime.create.result,
      error: state.DeliveryTime.create.error,
      loading: state.DeliveryTime.create.loading,
      visible: state.DeliveryTime.create.visible,
    },
    edit: {
      modelGet: state.DeliveryTime.edit.modelGet,
      modelPost: state.DeliveryTime.edit.modelPost,
      result: state.DeliveryTime.edit.result,
      error: state.DeliveryTime.edit.error,
      loading: state.DeliveryTime.edit.loading,
      visible: state.DeliveryTime.edit.visible,
    },
    deletes: {
      modelPost: state.DeliveryTime.deletes.modelPost,
      result: state.DeliveryTime.deletes.result,
      error: state.DeliveryTime.deletes.error,
      loading: state.DeliveryTime.deletes.loading,
      visible: state.DeliveryTime.deletes.visible,
    },
    activates: {
      modelPost: state.DeliveryTime.activates.modelPost,
      result: state.DeliveryTime.activates.result,
      error: state.DeliveryTime.activates.error,
    },
    grd: {
      loading: state.DeliveryTime.grd.loading,
    },
  };
}
