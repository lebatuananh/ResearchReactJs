...
class Ward extends React.Component {
  constructor(props) {
    ...
    this.frmSearch = {
      ...
      stateProvinceChange: (value) => {
        this.props.searchStateProvinceChange(value);
        this.frmSearch.ref.current.setFieldsValue({
          DistrictId: undefined,
        });
      },
    ...
    this.frmSearch.countryChange = this.frmSearch.countryChange.bind(this);
    this.frmSearch.stateProvinceChange = this.frmSearch.stateProvinceChange.bind(this);
    //#endregion
  }
      ...
      load: (loadOptions) => {
        ...
        const objParams = {
          ...
          ...{
            ...
            DistrictId: frmSearch.getFieldValue('DistrictId'),
            LanguageId: workContext.languageId(),
          },
        };
        let params = qs.stringify(objParams);
        return fetch(`${wardApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
  ...
  componentDidUpdate() {
    ...
    if (search.district.error) {
      notify.error(search.district.error);
    }
    if (create.data.district.error) {
      notify.error(create.data.district.error);
    }
    if (edit.data.district.error) {
      notify.error(edit.data.district.error);
    }
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
            ...
            <Col span={24} style={colStyle}>
              <Form id='frmSearch' ref={this.frmSearch.ref}>
                <Row gutter={gutter}>
                    ...
                    <Tooltip title={<FormattedMessage id='common.country' />}>
                      <Form.Item name='CountryId'>
                        <Select
                          ...
                          onChange={this.frmSearch.countryChange}
                        >
                    ...
                    <Tooltip title={<FormattedMessage id='common.stateProvince' />}>
                      <Form.Item name='StateProvinceId'>
                        <Select
                          ...
                          filterOption={selectHelper.def.filterOption}
                          onChange={this.frmSearch.stateProvinceChange}
                        >
                  ...
                  <Col span={5}>
                    <Tooltip title={<FormattedMessage id='common.district' />}>
                      <Form.Item name='DistrictId'>
                        <Select
                          ...
                          allowClear
                          showSearch
                          loading={search.district.loading}
                          filterOption={selectHelper.def.filterOption}
                        >
                          {search.district.data.map((item) => (
                            <Option key={item.Id} value={item.Id}>
                              {item.Name}
                            </Option>
                          ))}
          ...
          >
            <Column dataField='Code' dataType='string' caption={intl.formatMessage({ id: 'admin.wards.fields.code' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.wards.fields.name' })} fixed={true} width={220} />
            <Column dataField='ShortName' dataType='string' caption={intl.formatMessage({ id: 'common.fields.shortName' })} width={150} />
            <Column dataField='District.Name' dataType='string' caption={intl.formatMessage({ id: 'common.district' })} width={150} />
          ...
          <ContextMenu
            ...
            target='#grdWard .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.wards.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.wards.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    init: state.Ward.init,
    search: {
      countries: state.Ward.search.countries,
      stateProvince: {
        data: state.Ward.search.stateProvince.data,
        loading: state.Ward.search.stateProvince.loading,
        error: state.Ward.search.stateProvince.error,
      },
      district: {
        data: state.Ward.search.district.data,
        loading: state.Ward.search.district.loading,
        error: state.Ward.search.district.error,
      },
    },
    create: {
      modelGet: state.Ward.create.modelGet,
      modelPost: state.Ward.create.modelPost,
      result: state.Ward.create.result,
      error: state.Ward.create.error,
      loading: state.Ward.create.loading,
      visible: state.Ward.create.visible,
      data: {
        stateProvince: {
          error: state.Ward.create.data.stateProvince.error,
        },
        district: {
          error: state.Ward.create.data.district.error,
        },
      },
    },
    edit: {
      modelGet: state.Ward.edit.modelGet,
      modelPost: state.Ward.edit.modelPost,
      result: state.Ward.edit.result,
      error: state.Ward.edit.error,
      loading: state.Ward.edit.loading,
      visible: state.Ward.edit.visible,
      data: {
        stateProvince: {
          error: state.Ward.edit.data.stateProvince.error,
        },
        district: {
          error: state.Ward.edit.data.district.error,
        },
      },
    },
    deletes: {
      modelPost: state.Ward.deletes.modelPost,
      result: state.Ward.deletes.result,
      error: state.Ward.deletes.error,
      loading: state.Ward.deletes.loading,
      visible: state.Ward.deletes.visible,
    },
    activates: {
      modelPost: state.Ward.activates.modelPost,
      result: state.Ward.activates.result,
      error: state.Ward.activates.error,
    },
    grd: {
      loading: state.Ward.grd.loading,
    },
  };
}
function mapDispatchToProps(dispatch) {
  return {
    ...
    searchStateProvinceChange: (stateProvinceId) => dispatch(actions.searchStateProvinceChange(stateProvinceId)),
  };
}
