...
import actions from '../../../redux/list/sPMoveType/actions';
...
function preparePostModel(values) {
  if (values) {
    let _model = { ...values };
    _model['Air'] = false;
    _model['Ocean'] = false;
    _model['Inland'] = false;
    switch (_model['TransportMode']) {
      case 'Air':
        _model['Air'] = true;
        break;
      case 'Ocean':
...
class SPMoveType extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.frmCreate = {
      ...
      submit: () => {
        this.frmCreate.ref.current
          .validateFields()
          .then((values) => {
            //set airport, ocean, in land by type
            const _values = preparePostModel(values);
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${sPMoveTypeApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
          ...
          <Row style={rowStyle} gutter={gutter}>
            <Col span={9} style={colStyle}>
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <FormattedMessage id='common.sPMoveType' />
                </Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          ...
          >
            <Column dataField='Code' dataType='string' caption={intl.formatMessage({ id: 'admin.sPMoveTypes.fields.code' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.sPMoveTypes.fields.name' })} fixed={true} minWidth={220} />
            <Column dataField='LocalName' dataType='string' caption={intl.formatMessage({ id: 'common.fields.localName' })} minWidth={220} />
            <Column dataField='Air' cellRender={CellActive} dataType='boolean' caption={intl.formatMessage({ id: 'common.fields.air' })} width={100} />
            <Column dataField='Ocean' cellRender={CellActive} dataType='boolean' caption={intl.formatMessage({ id: 'common.fields.ocean' })} width={100} />
            <Column dataField='Inland' cellRender={CellActive} dataType='boolean' caption={intl.formatMessage({ id: 'common.fields.inland' })} width={100} />
            <Column
              dataField='Active'
              dataType='bool'
          ...
          <ContextMenu
            ...
            target='#grdSPMoveType .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.sPMoveTypes.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.sPMoveTypes.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    loading: state.SPMoveType.loading,
    create: {
      modelGet: state.SPMoveType.create.modelGet,
      modelPost: state.SPMoveType.create.modelPost,
      result: state.SPMoveType.create.result,
      error: state.SPMoveType.create.error,
      loading: state.SPMoveType.create.loading,
      visible: state.SPMoveType.create.visible,
    },
    edit: {
      modelGet: state.SPMoveType.edit.modelGet,
      modelPost: state.SPMoveType.edit.modelPost,
      result: state.SPMoveType.edit.result,
      error: state.SPMoveType.edit.error,
      loading: state.SPMoveType.edit.loading,
      visible: state.SPMoveType.edit.visible,
    },
    deletes: {
      modelPost: state.SPMoveType.deletes.modelPost,
      result: state.SPMoveType.deletes.result,
      error: state.SPMoveType.deletes.error,
      loading: state.SPMoveType.deletes.loading,
      visible: state.SPMoveType.deletes.visible,
    },
    activates: {
      modelPost: state.SPMoveType.activates.modelPost,
      result: state.SPMoveType.activates.result,
      error: state.SPMoveType.activates.error,
    },
    grd: {
      loading: state.SPMoveType.grd.loading,
    },
  };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(SPMoveType);
