...
import actions from '../../../redux/list/bankAccount/actions';
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${bankAccountApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
          ...
          <Row style={rowStyle} gutter={gutter}>
            <Col span={9} style={colStyle}>
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <FormattedMessage id='common.bankAccount' />
                </Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          ...
          >
            <Column dataField='AccountNumber' dataType='string' caption={intl.formatMessage({ id: 'common.fields.accountNumber' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.bankAccounts.fields.name' })} fixed={true} width={220} />
            <Column dataField='Bank.Name' dataType='string' caption={intl.formatMessage({ id: 'common.bank' })} fixed={true} width={220} />
            <Column dataField='LocalName' dataType='string' caption={intl.formatMessage({ id: 'common.fields.localName' })} width={220} />
            <Column dataField='BankBranch.Name' dataType='string' caption={intl.formatMessage({ id: 'common.bankBranch' })} width={150} />
            <Column dataField='AccountMasterId' dataType='string' caption={intl.formatMessage({ id: 'common.fields.accountMaster' })} width={220} />
            {/* <Column dataField='Currency.Name' dataType='string' caption={intl.formatMessage({ id: 'common.currency' })} width={150} /> */}
            {/* <Column dataField='IBAN' dataType='string' caption={intl.formatMessage({ id: 'admin.bankAccounts.fields.IBAN' })} width={150} /> */}
          ...
          <ContextMenu
            ...
            target='#grdBankAccount .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.bankAccounts.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.bankAccounts.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    create: {
      modelGet: state.BankAccount.create.modelGet,
      modelPost: state.BankAccount.create.modelPost,
      result: state.BankAccount.create.result,
      error: state.BankAccount.create.error,
      loading: state.BankAccount.create.loading,
      visible: state.BankAccount.create.visible,
    },
    edit: {
      modelGet: state.BankAccount.edit.modelGet,
      modelPost: state.BankAccount.edit.modelPost,
      result: state.BankAccount.edit.result,
      error: state.BankAccount.edit.error,
      loading: state.BankAccount.edit.loading,
      visible: state.BankAccount.edit.visible,
    },
    deletes: {
      modelPost: state.BankAccount.deletes.modelPost,
      result: state.BankAccount.deletes.result,
      error: state.BankAccount.deletes.error,
      loading: state.BankAccount.deletes.loading,
      visible: state.BankAccount.deletes.visible,
    },
    activates: {
      modelPost: state.BankAccount.activates.modelPost,
      result: state.BankAccount.activates.result,
      error: state.BankAccount.activates.error,
    },
    grd: {
      loading: state.BankAccount.grd.loading,
    },
  };
}
