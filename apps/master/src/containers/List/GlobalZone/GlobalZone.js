...
import actions from '../../../redux/list/globalZone/actions';
...
class GlobalZone extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${globalZoneApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
          ...
          <Row style={rowStyle} gutter={gutter}>
            <Col span={9} style={colStyle}>
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <FormattedMessage id='common.globalZone' />
                </Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          ...
          >
            <Column dataField='Code' dataType='string' caption={intl.formatMessage({ id: 'admin.globalZones.fields.code' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.globalZones.fields.name' })} fixed={true} width={220} />
          ...
          <ContextMenu
            ...
            target='#grdGlobalZone .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.globalZones.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.globalZones.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    loading: state.GlobalZone.loading,
    create: {
      modelGet: state.GlobalZone.create.modelGet,
      modelPost: state.GlobalZone.create.modelPost,
      result: state.GlobalZone.create.result,
      error: state.GlobalZone.create.error,
      loading: state.GlobalZone.create.loading,
      visible: state.GlobalZone.create.visible,
    },
    edit: {
      modelGet: state.GlobalZone.edit.modelGet,
      modelPost: state.GlobalZone.edit.modelPost,
      result: state.GlobalZone.edit.result,
      error: state.GlobalZone.edit.error,
      loading: state.GlobalZone.edit.loading,
      visible: state.GlobalZone.edit.visible,
    },
    deletes: {
      modelPost: state.GlobalZone.deletes.modelPost,
      result: state.GlobalZone.deletes.result,
      error: state.GlobalZone.deletes.error,
      loading: state.GlobalZone.deletes.loading,
      visible: state.GlobalZone.deletes.visible,
    },
    activates: {
      modelPost: state.GlobalZone.activates.modelPost,
      result: state.GlobalZone.activates.result,
      error: state.GlobalZone.activates.error,
    },
    grd: {
      loading: state.GlobalZone.grd.loading,
    },
  };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(GlobalZone);
