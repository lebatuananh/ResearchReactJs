...
class ShippingLine extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.grd.dataSource = new CustomStore({
      ...
      load: (loadOptions) => {
        ...
        return fetch(`${shippingLineApi.get}?${params}`, { headers: headersHelper.def })
          .then(grd.errors)
    ...
    return (
      <Spin spinning={loading} size='large'>
        <LayoutContentWrapper>
          ...
          <Row style={rowStyle} gutter={gutter}>
            <Col span={9} style={colStyle}>
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <FormattedMessage id='common.shippingLine' />
                </Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          ...
          >
            <Column dataField='Code' dataType='string' caption={intl.formatMessage({ id: 'admin.shippingLines.fields.code' })} fixed={true} width={150} />
            <Column dataField='Name' dataType='string' caption={intl.formatMessage({ id: 'admin.shippingLines.fields.name' })} fixed={true} width={220} />
            <Column dataField='SCAC' dataType='string' caption={intl.formatMessage({ id: 'common.fields.SCAC' })} width={220} />
            <Column dataField='LocalName' dataType='string' caption={intl.formatMessage({ id: 'common.fields.localName' })} width={220} />
            <Column dataField='ShippingAgent.Name' dataType='string' caption={intl.formatMessage({ id: 'common.shippingAgent' })} width={220} />
          ...
          <ContextMenu
            ...
            target='#grdShippingLine .dx-datagrid-table tbody .dx-row.dx-data-row'
            width={180}
          ...
          <Modal
            title={<FormattedMessage id='admin.shippingLines.create' />}
          ...
          <Modal
            title={<FormattedMessage id='admin.shippingLines.edit' />}
...
function mapStateToProps(state) {
  return {
    ...
    loading: state.ShippingLine.loading,
    create: {
      modelGet: state.ShippingLine.create.modelGet,
      modelPost: state.ShippingLine.create.modelPost,
      result: state.ShippingLine.create.result,
      error: state.ShippingLine.create.error,
      loading: state.ShippingLine.create.loading,
      visible: state.ShippingLine.create.visible,
      visibleSubmit: state.ShippingLine.create.visibleSubmit,
    },
    edit: {
      modelGet: state.ShippingLine.edit.modelGet,
      modelPost: state.ShippingLine.edit.modelPost,
      result: state.ShippingLine.edit.result,
      error: state.ShippingLine.edit.error,
      loading: state.ShippingLine.edit.loading,
      visible: state.ShippingLine.edit.visible,
      visibleSubmit: state.ShippingLine.edit.visibleSubmit,
    },
    deletes: {
      modelPost: state.ShippingLine.deletes.modelPost,
      result: state.ShippingLine.deletes.result,
      error: state.ShippingLine.deletes.error,
      loading: state.ShippingLine.deletes.loading,
      visible: state.ShippingLine.deletes.visible,
    },
    activates: {
      modelPost: state.ShippingLine.activates.modelPost,
      result: state.ShippingLine.activates.result,
      error: state.ShippingLine.activates.error,
    },
    grd: {
      loading: state.ShippingLine.grd.loading,
    },
  };
}
